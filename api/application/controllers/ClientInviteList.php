...
class ClientInviteList extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('ClientInviteList_model');
	...
	public function getAll() {
		...
		$data['Inv']=$this->ClientInviteList_model->getlist_Invitation();
		
        $data['status']=$this->ClientInviteList_model->get_invimsg();
	...
	public function delete() {
		...
		if ($post_revoke)
		 {
			if($post_revoke['id'] > 0){
				$result = $this->ClientInviteList_model->delete_Invitation($post_revoke);
				if($result) {
	...
	public function ReInvite() {
		...
		if(!empty($post_Invitation)) {
			...
			$result = $this->ClientInviteList_model->ReInvite_Invitation($post_Invitation);			
			if($result) {
				...
				foreach($query->result() as $row){ 
					if($row->To==4){
						...
						//$body = str_replace("{action_url}",''.BASE_URL.'/client-registration/'.JWT::encode($post_Invitation['InvitationCode'],"MyGeneratedKey","HS256").'',$body);
						$body = str_replace("{action_url}",''.BASE_URL.'/'.$WorkspaceURL.'/client-registration/'.JWT::encode($post_Invitation['InvitationCode'],"MyGeneratedKey","HS256").'',$body);
	...
	public function invimsg() {
		$data=$this->ClientInviteList_model->get_invimsg();
		echo json_encode($data);

...
class Rolepermission_model extends CI_Model
 {
	...
	public function getpermissionlist($role_id = NULL)
	{
		try{
		if($role_id){
			$query = "SELECT s.ScreenId,s.DisplayName,s.Name,s.IsActive,r.PermissionId,(CASE WHEN r.AddEdit = 1 THEN 'true' ELSE NULL END) AS AddEdit,(CASE WHEN r.Delete = 1 THEN 'true' ELSE NULL END) AS dlt,(CASE WHEN r.View = 1 THEN 'true' ELSE NULL END) AS View FROM tblmstscreen as s LEFT JOIN tblrolespermission as r ON s.ScreenId = r.ScreenId WHERE s.IsActive = 1 AND r.RoleId = ".$role_id;
